...
Public Class frmGLGroup
    ...
    Private Sub Button3_Click(sender As Object, e As EventArgs) Handles cmdSave.Click
        If txtItemName.Text = "" Then
            ...
            txtItemName.Focus()
            Exit Sub
        ElseIf countqry("tblglgroup", "groupname='" & rchar(txtItemName.Text) & "'") > 0 And mode.Text <> "edit" Then
        ...
        If mode.Text = "edit" Then
            com.CommandText = "update tblglgroup set groupname='" & rchar(txtItemName.Text) & "'  where groupcode='" & groupcode.Text & "'" : com.ExecuteNonQuery()
        Else
            com.CommandText = "insert into tblglgroup set  groupcode='" & groupcode.Text & "',groupname='" & rchar(txtItemName.Text) & "' " : com.ExecuteNonQuery()
        End If
    ...
    Public Sub FilterItem()
        Try
            ...
            msda = New MySqlDataAdapter("select groupcode as Code,groupname as 'Group Name' from tblglgroup where groupname like '%" & rchar(txtSearch.Text) & "%' order by groupcode asc", conn)
            msda.Fill(dst, 0)
            Em.DataSource = dst.Tables(0)
            GridColumnAlignment(Em, {"Code"}, DataGridViewContentAlignment.MiddleCenter)
            GridColumnWidth(Em, {"Code"}, 50)
    ...
    Private Sub EditChapterToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles EditChapterToolStripMenuItem.Click
        com.CommandText = "select * from tblglgroup where groupcode='" & Em.Item("Code", Em.CurrentRow.Index).Value() & "'" : rst = com.ExecuteReader
        While rst.Read
    ...
    Private Sub TagAsInactiveMemberToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles TagAsInactiveMemberToolStripMenuItem.Click
        If MessageBox.Show("Are you sure you want to delete selected item?", Me.Text, MessageBoxButtons.YesNo, MessageBoxIcon.Question) = vbYes Then
            com.CommandText = "delete from tblglgroup where groupcode='" & Em.Item("Code", Em.CurrentRow.Index).Value() & "'" : com.ExecuteNonQuery()
            FilterItem()
